...
for TYPE in dashboard visualization index-pattern; do
...
for obj in r.get('saved_objects', []):
    ...
    if '${ID}' in title or '01-' in obj.get('id', ''):
        print(obj['id'])
" 2>/dev/null || true)
  for id in $IDS; do
...
echo "[${ID}] Deleting alert rules for Scenario 01..."
...
for rule in r.get('data', []):
    if 'Scenario 01' in rule.get('name', '') or 'Auth Brute Force' in rule.get('name', ''):
        print(rule['id'])
" 2>/dev/null || true)
